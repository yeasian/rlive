...
新用户可免费试用 1 天；需要继续使用时，可在「系统设置」中输入激活码完成激活。一个激活码支持激活两台设备。
...
请先安装 Microsoft Visual C++ 运行库（`VC_redist.x64.exe`），并尽量将软件安装到不含空格的路径中。

### 没有设置分段，为什么视频仍然被分成多个文件？

主播短暂断播、直播源切换或本地网络中断，都可能让一次录制生成多个文件。

### 录制内容频繁卡顿怎么办？

请检查本地网络和主播直播源是否稳定。并行录制会占用较多带宽，如出现卡顿，建议减少同时录制的任务数量（通常控制在 5 个以内）。

### 提示“时间不准确，请校准本地时间”怎么办？

打开 Windows 日期和时间设置，启用自动设置时间并立即同步，然后重新打开 RLive。

## 下载与帮助

- [下载 RLive](https://share.feijipan.com/s/0pXePiwt?code=7894)（提取码：`7894`）
- [查看完整使用指南](https://sw0sj62e43t.feishu.cn/wiki/HZpQw8Hb8iQ21Ok0puEcS9MvnSg?from=from_copylink)
- 软件内可通过「联系&指南」查看最新教程和联系方式
